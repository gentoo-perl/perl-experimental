...
dual_scripts() {
	...
	src_remove_dual_scripts 1.38 ptar ptardiff
	# - perl-core/Digest-SHA
	src_remove_dual_scripts 5.45 shasum
	# - perl-core/CPAN
	src_remove_dual_scripts 1.9205 cpan
	# - perl-core/CPANPLUS
	src_remove_dual_scripts 0.84 cpanp cpan2dist cpanp-run-perl
	# - perl-core/Encode
	src_remove_dual_scripts 2.23 enc2xs piconv
	# - perl-core/ExtUtils-MakeMaker
	src_remove_dual_scripts 6.42 instmodsh
	# - perl-core/Module-Build
	src_remove_dual_scripts 0.2808_01 config_data
	# - perl-core/Module-CoreList
	src_remove_dual_scripts 2.12 corelist
	# - perl-core/PodParser
	src_remove_dual_scripts 1.35 pod2usage podchecker podselect
	# - perl-core/Test-Harness
	src_remove_dual_scripts 2.64 prove
}
...
src_remove_dual_scripts() {
	...
	if has "${EBUILD_PHASE:-none}" "postinst" "postrm" ;then
		for i in "$@" ; do
			ff=`echo /usr/share/man/man1/${i}-${ver}-${P}.1*`
			ff=${ff##*.1}
...
pkg_postinst() {
	...
	if [[ "${ROOT}" = "/" ]] ; then
		...
		cd /usr/include
		h2ph *
		h2ph -r sys/* arpa/* netinet/* bits/* security/* asm/* gnu/* linux/*
